...
class CouplingConfig:
    ...
    Args:
        mode: 커플링 모드 ("manual" 또는 "auto")
        particle_method: PD 영역 솔버 ("pd" 또는 "spg")
        pd_element_indices: 수동 모드 — PD 영역 요소 인덱스
        criteria: 자동 모드 — 전환 기준 딕셔너리
            {"von_mises_threshold": float, "max_strain_threshold": float,
             "buffer_layers": int}
    ...
    pd_element_indices: Optional[list] = None
...
class Domain:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._fixed_dofs: Optional[list] = None  # Per-DOF 고정 (None=전체)
        ...
        self._coupling_config: Optional[CouplingConfig] = None
