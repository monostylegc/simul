...
class Material:
    ...
    Args:
        ...
        yield_stress: J2 소성 항복 응력 [Pa]
        hardening_modulus: J2 소성 경화 계수 [Pa]
    """
    E: float
    nu: float
    ...
    # J2 소성 파라미터 (선택적)
    yield_stress: Optional[float] = None        # 항복 응력 σ_y₀
    hardening_modulus: Optional[float] = None    # 경화 계수 H

    # 횡이방성 파라미터 (선택적)
    E1: Optional[float] = None       # 이방 축 영 계수
    E2: Optional[float] = None       # 등방면 영 계수
    nu12: Optional[float] = None     # 이방 축 ↔ 등방면 푸아송 비
    nu23: Optional[float] = None     # 등방면 내 푸아송 비
    G12: Optional[float] = None      # 이방 축 ↔ 등방면 전단 계수
    fiber_direction: Optional[tuple] = None  # 이방 축 방향 벡터
    ...
    def __post_init__(self):
        ...
        # 기본 탄성 상수 검증
        from ..fem.validation import validate_elastic_constants, validate_density
        validate_elastic_constants(self.E, self.nu, "Material")
        validate_density(self.density, "Material")
    ...
    def _create_fem_material(self):
        ...
        elif self.constitutive_model == "j2_plasticity":
            from ..fem.material.j2_plasticity import J2Plasticity
            return J2Plasticity(
                youngs_modulus=self.E,
                poisson_ratio=self.nu,
                yield_stress=self.yield_stress or 880e6,
                hardening_modulus=self.hardening_modulus or 0.0,
                dim=self.dim,
            )
        elif self.constitutive_model == "transverse_isotropic":
            from ..fem.material.transverse_isotropic import TransverseIsotropic
            return TransverseIsotropic(
                E1=self.E1 or self.E,
                E2=self.E2 or self.E,
                nu12=self.nu12 or self.nu,
                nu23=self.nu23 or self.nu,
                G12=self.G12 or (self.E / (2.0 * (1.0 + self.nu))),
                fiber_direction=self.fiber_direction,
                dim=self.dim,
            )
        else:
            # 기본값: linear_elastic
