...
class PDAdapter(AdapterBase):
    ...
    def __init__(self, domain: Domain, material: Material, **options):
        ...
        # 이웃 탐색 — 적응적 할당: 사전 카운트 → max_bonds 자동 설정
        ...
        # 1단계: 이웃 수 사전 카운트 (max_neighbors 제한 없이)
        ns_scan = NeighborSearch(
            domain_min=tuple(mins),
            domain_max=tuple(maxs),
            horizon=horizon,
            max_particles=n_particles,
            max_neighbors=4,  # 카운트 전용이므로 최소값
            dim=dim,
        )
        neighbor_counts = ns_scan.count_neighbors_only(self.ps.X, n_particles)
        max_neighbors = int(neighbor_counts.max()) + 8  # 안전 여유분

        # 2단계: 실제 이웃 탐색 (적응적 max_neighbors)
        ns = NeighborSearch(
            domain_min=tuple(mins),
            domain_max=tuple(maxs),
            horizon=horizon,
        ...
        # 본드 시스템 (적응적 할당)
        self.bonds = BondSystem.from_neighbor_counts(
            n_particles, neighbor_counts, dim=dim, margin=8
        )
