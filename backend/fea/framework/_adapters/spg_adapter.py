...
class SPGAdapter(AdapterBase):
    ...
    def __init__(self, domain: Domain, material: Material, **options):
        ...
        # 커널 — 적응적 할당: 이웃 수 사전 카운트 → max_neighbors 자동 설정
        # cKDTree.query_ball_point(return_length=True): 카운트만 반환, O(n) 메모리
        from scipy.spatial import cKDTree
        positions_np = self.ps.X.to_numpy()
        tree = cKDTree(positions_np)
        neighbor_counts_raw = tree.query_ball_point(
            positions_np, support_radius, return_length=True
        )
        neighbor_counts = (np.asarray(neighbor_counts_raw, dtype=np.int32) - 1)  # 자기 자신 제외

        self.kernel = SPGKernel.from_neighbor_counts(
        ...
        self.kernel.build_neighbor_list(positions_np, support_radius)
        self.kernel.build_reverse_map()  # gather 방식 내부력 계산 활성화
        self.kernel.compute_shape_functions(self.ps.X, self.ps.volume)

        # 본드 시스템 (적응적 할당)
        self.bonds = SPGBondSystem.from_neighbor_counts(
            n_particles=n_particles,
            counts=neighbor_counts,
            dim=dim,
            margin=8,
