...
class SPGCompute:
    ...
    @ti.kernel
    def compute_internal_force_gather(self):
        """Galerkin DNI 내부력 계산 (gather 방식, atomic_add 제거).

        scatter 방식과 수학적으로 동일한 결과를 산출하지만
        atomic_add 없이 각 입자가 자신의 기여분을 직접 수집한다.

        입자 j의 내부력:
          f_j = 자기 기여 + 이웃 기여
          자기 기여: -σ_j · (Σ_k dpsi[j,k]) · V_j
          이웃 기여: Σ_m σ_{reverse_i[j,m]} · dpsi[reverse_i[j,m], reverse_k[j,m]] · V_{reverse_i[j,m]}

        역방향 맵이 사전에 구축되어 있어야 한다 (kernel.build_reverse_map()).
        """
        for j in range(self.n_particles):
            # 1단계: 자기 기여 (self-contribution)
            dpsi_sum = ti.Vector.zero(ti.f64, self.dim)
            n_nbr = self.kernel.n_neighbors[j]
            for k in range(n_nbr):
                if self.bonds.broken[j, k] == 0:
                    dpsi_sum += self.kernel.dpsi[j, k]

            sigma_j = self.particles.stress[j]
            V_j = self.particles.volume[j]
            f_j = -(sigma_j @ dpsi_sum) * V_j

            # 2단계: 역방향 이웃으로부터 기여 수집 (gather)
            # reverse_i[j, m] = i: 입자 i가 j를 이웃으로 가짐
            # reverse_k[j, m] = k: neighbors[i, k] == j
            # 기여: σ_i · dpsi[i, k] · V_i
            n_rev = self.kernel.n_reverse[j]
            for m in range(n_rev):
                i = self.kernel.reverse_i[j, m]
                k = self.kernel.reverse_k[j, m]
                if self.bonds.broken[i, k] == 0:
                    sigma_i = self.particles.stress[i]
                    V_i = self.particles.volume[i]
                    dpsi_j_at_i = self.kernel.dpsi[i, k]
                    f_j += sigma_i @ dpsi_j_at_i * V_i

            self.particles.f_int[j] = f_j
    ...
    def compute_internal_force_with_stabilization(self):
        ...
        역방향 맵이 구축되어 있으면 gather 방식 사용 (GPU 경합 제거),
        아니면 기존 scatter 방식으로 폴백한다.

        재료 상수는 particles.lam_param, particles.mu_param에서 읽는다.
        사전에 particles.set_material_constants() 호출 필요.
        """
        self.compute_stress()
        if self.kernel._reverse_built:
            self.compute_internal_force_gather()
        else:
