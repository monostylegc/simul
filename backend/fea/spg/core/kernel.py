...
class SPGKernel:
    ...
    def from_neighbor_counts(
        ...
        max_neighbors = max(counts) + margin 으로 자동 설정.

        Args:
            n_particles: 입자 수
            counts: 각 입자의 이웃 수 (n_particles,)
            dim: 공간 차원
            support_radius: 커널 지지 반경
            margin: 안전 여유분

        Returns:
            적응적으로 할당된 SPGKernel
        """
        max_neighbors = int(np.max(counts)) + margin
        return cls(n_particles, max_neighbors=max_neighbors,
                   dim=dim, support_radius=support_radius)
    ...
    def __init__(
        ...
        """
        # 지지 반경 검증
        from ...fem.validation import validate_support_radius
        validate_support_radius(support_radius)
        ...
        # 역방향 이웃 맵 (gather 방식 내부력 계산용)
        # reverse_i[j, m] = 입자 i (j를 이웃으로 가진 입자)
        # reverse_k[j, m] = neighbors[i, k] == j 인 k 인덱스
        # n_reverse[j] = j를 이웃으로 가진 입자 수
        self.reverse_i = ti.field(dtype=ti.i32, shape=(n_particles, max_neighbors))
        self.reverse_k = ti.field(dtype=ti.i32, shape=(n_particles, max_neighbors))
        self.n_reverse = ti.field(dtype=ti.i32, shape=n_particles)
    ...
    def build_reverse_map(self):
        """역방향 이웃 맵 구축 (gather 방식 내부력 계산의 전제조건).

        순방향: neighbors[i, k] = j → 입자 i가 j를 이웃으로 가짐
        역방향: reverse_i[j, m] = i, reverse_k[j, m] = k
                → 입자 j를 이웃으로 가진 입자 i와 그 인덱스 k

        CPU에서 구축 후 Taichi 필드에 복사 (1회성 전처리).
        """
        n = self.n_particles
        neighbors_np = self.neighbors.to_numpy()
        n_neighbors_np = self.n_neighbors.to_numpy()

        # 역방향 맵 구축 (CPU)
        reverse_i_np = np.zeros((n, self.max_neighbors), dtype=np.int32)
        reverse_k_np = np.zeros((n, self.max_neighbors), dtype=np.int32)
        n_reverse_np = np.zeros(n, dtype=np.int32)

        for i in range(n):
            for k in range(n_neighbors_np[i]):
                j = neighbors_np[i, k]
                m = n_reverse_np[j]
        ...
        self.reverse_i.from_numpy(reverse_i_np)
        self.reverse_k.from_numpy(reverse_k_np)
        self.n_reverse.from_numpy(n_reverse_np)
