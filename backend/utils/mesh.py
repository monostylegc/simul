...
class TriangleMesh:
    ...
    def save_stl(self, filepath: str, binary: bool = True):
        ...
        Args:
            filepath: 저장 경로
            binary: True면 바이너리 STL, False면 ASCII STL
        """
        filepath = Path(filepath)
        ...
        if binary:
            self._save_stl_binary(filepath, verts)
        else:
            self._save_stl_ascii(filepath, verts)

    def _save_stl_binary(self, filepath: Path, verts: np.ndarray):
        """바이너리 STL 저장."""
        with open(filepath, 'wb') as f:
            # 80바이트 헤더
            header = f"Binary STL - {self.name}".encode('ascii')
            header = header[:80].ljust(80, b'\0')
            f.write(header)

            # 삼각형 개수
            n_triangles = len(self.faces)
            f.write(np.array([n_triangles], dtype=np.uint32).tobytes())
    ...
    def _save_stl_ascii(self, filepath: Path, verts: np.ndarray):
        """ASCII STL 저장."""
    ...
    def save_obj(self, filepath: str):
        ...
        with open(filepath, 'w') as f:
            f.write(f"# OBJ file - {self.name}\n")
            f.write(f"# Vertices: {len(verts)}, Faces: {len(self.faces)}\n\n")
    ...
    def save(self, filepath: str, **kwargs):
        ...
        Args:
            filepath: 저장 경로 (.stl 또는 .obj)
            **kwargs: save_stl 또는 save_obj에 전달할 추가 인자
        """
        filepath = Path(filepath)
        ext = filepath.suffix.lower()

        if ext == '.stl':
            self.save_stl(filepath, **kwargs)
        elif ext == '.obj':
            self.save_obj(filepath, **kwargs)
        else:
            raise ValueError(f"지원하지 않는 포맷: {ext}")

    @classmethod
    def merge_meshes(cls, meshes: List["TriangleMesh"], name: str = "merged") -> "TriangleMesh":
        """여러 메쉬를 하나로 병합.
