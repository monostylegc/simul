...
class DynamicSolver:
    ...
    def __init__(
        ...
        self._static_helper = StaticSolver(mesh, material)
    ...
    def _compute_lumped_mass(self, density: float) -> np.ndarray:
        ...
        for e in range(self.mesh.n_elements):
            mass_per_node = density * elem_vol[e] / nodes_per_elem
            for a in range(nodes_per_elem):
                node = elements[e, a]
                node_mass[node] += mass_per_node

        # DOF별 질량 (각 방향 동일)
        M_diag = np.zeros(self.n_dof)
        for i in range(n_nodes):
            for d in range(dim):
                M_diag[i * dim + d] = node_mass[i]
